...
void onMouse1(int event, int x, int y, int foo, void* p)
{
	...
	// Computes two points on the epipolar line and draw it
	Point m2a(0, -l(2)/l(1));
	Point m2b(D->I2.width(), -(l(2) + l(0) * D->I2.width())/l(1));
	line(D->I2, m2a, m2b, Scalar(0,255,0),1);

	// Finds the point on the epiplar line that best correlates with the clicked point and draw it
	double max_similarity = NCC(D->F1, m1, D->F2, m2a, 10);
	Point best_m2 = m2a;
	for (double x = 0; x <= D->I2.width(); x++){
		Point m2 (x, -(l(2) + l(0) * x)/l(1));
		double similarity = NCC(D->F1, m1, D->F2, m2, 4);
		if (similarity > max_similarity){
			max_similarity =	similarity;
			best_m2 = m2;
		}
	}
	circle(D->I2,best_m2,2,Scalar(0,255,0),2);
...
void onMouse2(int event, int x, int y, int foo, void* p)
{
	...
	// Computes two points on the epipolar line and draw it
	Point m1a(0, -l(2)/l(1));
	Point m1b(D->I1.width(), -(l(2) + l(0) * D->I1.width())/l(1));
	line(D->I1, m1a, m1b, Scalar(0, 0, 255),1);

	// Finds the point on the epiplar line that best correlates with the clicked point and draw it
	double max_similarity = NCC(D->F2, m2, D->F1, m1a, 10);
	Point best_m1 = m1a;
	for (double x = 0; x <= D->I1.width(); x++){
		Point m1 (x, -(l(2) + l(0) * x)/l(1));
		double similarity = NCC(D->F2, m2, D->F1, m1, 4);
		if (similarity > max_similarity){
			max_similarity =	similarity;
			best_m1 = m1;
		}
	}
	circle(D->I1,best_m1,2,Scalar(0, 0, 255),2);
